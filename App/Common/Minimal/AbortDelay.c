...
void vCreateAbortDelayTasks( void )
{
	...
	xTaskCreate( prvControllingTask, pcControllingTaskName, tskSTACK_SIZE, NULL, abtCONTROLLING_PRIORITY, NULL );
	xTaskCreate( prvBlockingTask, pcBlockingTaskName, tskSTACK_SIZE, NULL, abtBLOCKING_PRIORITY, NULL );
}
/*-----------------------------------------------------------*/
...
const TickType_t xStartMargin = 2UL;
	...
	for( ;; )
	{
		...
		xControllingCycles++;
		portDEBUG_PRINTF("AbortDelay controlling cycles %d\n", xControllingCycles);
	}
}
/*-----------------------------------------------------------*/
...
const uint32_t ulMax = 0xffffffffUL;
	...
	for( ;; )
	{
		...
		/* To indicate this task is still executing. */
		xBlockingCycles++;
		portDEBUG_PRINTF("AbortDelay blockingcycles %d\n", xBlockingCycles);
	}
}
/*-----------------------------------------------------------*/
