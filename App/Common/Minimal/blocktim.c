...
{
	portDEBUG_PRINTF("creating blocktime tasks\n");
...
TickType_t xTimeToBlock, xBlockedTime;
	...
	for( ;; )
	{
		...
		portDEBUG_PRINTF("BasicDelay done\n");
		...
		for( xItem = 0; xItem < bktQUEUE_LENGTH; xItem++ )
		{
			...
			portDEBUG_PRINTF("Time when blocking %d\n", xTimeWhenBlocking);
			portDEBUG_PRINTF("Time to block %d\n", xTimeToBlock);
			...
			portDEBUG_PRINTF("Time after blocking %d\n", xTimeWhenBlocking);
		...
		for( xItem = 0; xItem < bktQUEUE_LENGTH; xItem++ )
		{
			...
			portDEBUG_PRINTF("Time when blocking 2 %d\n", xTimeWhenBlocking);
			/* We should unblock after xTimeToBlock having not received
			anything on the queue. */
...
const TickType_t xPeriod = 75, xCycles = 5, xAllowableMargin = ( bktALLOWABLE_MARGIN >> 1 );
	...
	portDEBUG_PRINTF("Basic delay tests\n");
	...
	portDEBUG_PRINTF("PreTime %04x Posttime %04x\n", xPreTime, xPostTime);
	...
	for( x = 0; x < xCycles; x++ )
	{
		...
		portDEBUG_PRINTF("Expcted unlock time  %d\n", xExpectedUnblockTime);

		vTaskDelayUntil( &xLastUnblockTime, xPeriod );

		portDEBUG_PRINTF("Back  %d\n", xTaskGetTickCount());
	...
	portDEBUG_PRINTF("lower priority to  %d\n", bktPRIMARY_PRIORITY);
	/* Reset to the original task priority ready for the other tests. */
	// PS vTaskPrioritySet( NULL, bktPRIMARY_PRIORITY );
}
/*-----------------------------------------------------------*/
