...
uint8_t ucTimer;
	...
	if( xTestStatus == pdPASS )
	{
		...
		ulLoopCounter++;
		portDEBUG_PRINTF("Timer can be stopped loop %d\n", ulLoopCounter);
	}
}
/*-----------------------------------------------------------*/
...
static void prvTest5_CheckBasicOneShotTimerBehaviour( void )
{
	...
	if( xTestStatus == pdPASS )
	{
		...
		check task knows this task is still running. */
		ulLoopCounter++;
		portDEBUG_PRINTF("One Shot Timer loop %d\n", ulLoopCounter);
	}
}
/*-----------------------------------------------------------*/
...
uint8_t ucTimer;
	...
	for( ucTimer = 0; ucTimer < trmdemoNUM_TIMER_RESETS; ucTimer++ )
	{
		...
		if( xTestStatus == pdPASS )
		{
			...
			the check task knows this task is still running. */
			ulLoopCounter++;
			portDEBUG_PRINTF("Autoreload Timer loop %d\n", ulLoopCounter);
		}
	}
	...
	if( xTestStatus == pdPASS )
	{
		...
		ulLoopCounter++;
		portDEBUG_PRINTF("Autoreload Timer final %d\n", ulLoopCounter);
	}
}
/*-----------------------------------------------------------*/
