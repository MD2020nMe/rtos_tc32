...
static QueueHandle_t xPolledQueue;
	...
	if( xPolledQueue != NULL )
	{
		...
		portDEBUG_PRINTF("Starting poll test\n");
			...
			else
			{
				if( xError == pdFALSE )
				{
					...
					portENTER_CRITICAL();
						...
						portDEBUG_PRINTF("Producer count %d\n", (uint32_t)xPollingProducerCount);
					portEXIT_CRITICAL();
				}
				...
				else
				{
					if( xError == pdFALSE )
					{
						...
						portENTER_CRITICAL();
							...
							portDEBUG_PRINTF("Consumer count %d\n", (uint32_t) xPollingConsumerCount);
						portEXIT_CRITICAL();
					}
				}
