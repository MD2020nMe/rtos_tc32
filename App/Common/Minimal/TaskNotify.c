...
#define notifyTASK_PRIORITY		( tskIDLE_PRIORITY  + 1)
...
void vStartTaskNotifyTask( void  )
{
	...
	xTaskCreate( prvNotifiedTask, "Notified", tskSTACK_SIZE, NULL, notifyTASK_PRIORITY, &xTaskToNotify );
...
const uint32_t ulBit0 = 0x01UL, ulBit1 = 0x02UL;
	...
	portDEBUG_PRINTF("SingleTaskTest TimeOnEntering %d\n", xTimeOnEntering);
	...
	ulNotifyCycleCount++;
	portDEBUG_PRINTF("NotifyCycleCount  %d\n", ulNotifyCycleCount);
...
static void prvNotifyingTimer( TimerHandle_t xNotUsed )
{
	...
	taskENTER_CRITICAL();
	{
		...
		portDEBUG_PRINTF("NotifyingTImert  %d\n", ulTimerNotificationsSent);
	}
	taskEXIT_CRITICAL();
}
/*-----------------------------------------------------------*/
