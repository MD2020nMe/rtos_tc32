...
#define tskSTACK_SIZE 		configMINIMAL_STACK_SIZE
#define LED_MUTEX_TESTING  	1
...
void vStartRecursiveMutexTasks( void )
{
	...
	if( xMutex != NULL )
	{
		...
		xTaskCreate( prvRecursiveMutexControllingTask, "Rec1", tskSTACK_SIZE, NULL, recmuCONTROLLING_TASK_PRIORITY, &xControllingTaskHandle );
		xTaskCreate( prvRecursiveMutexBlockingTask, "Rec2", tskSTACK_SIZE, NULL, recmuBLOCKING_TASK_PRIORITY, &xBlockingTaskHandle );
		xTaskCreate( prvRecursiveMutexPollingTask, "Rec3", tskSTACK_SIZE, NULL, recmuPOLLING_TASK_PRIORITY, NULL );
	}
}
/*-----------------------------------------------------------*/
...
UBaseType_t ux;
	...
	for( ;; )
	{
		...
		uxControllingCycles++;
		portDEBUG_PRINTF("Mutex controlling cycle counts %d\n", uxControllingCycles);
...
static void prvRecursiveMutexBlockingTask( void *pvParameters )
{
	...
	for( ;; )
	{
		...
		stall can be detected. */
		uxBlockingCycles++;
		portDEBUG_PRINTF("Mutex blocking cycle counts %d\n", uxBlockingCycles);
	}
}
/*-----------------------------------------------------------*/
