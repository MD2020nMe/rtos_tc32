...
*
*/
 void setPGAOutput(unsigned char enDis){
...
void Audio_BatteryCheckInit(unsigned char checkM){//add to audio_Init
	...
	write_reg8(0x80002e,0x12); //select input for R-channel
    write_reg8(0x80002c,0x12); //select input for M-channel
	...
	SETB(0x800033, 0x02);// Enable M-channel
	SETB(0x800033, 0x08);// Enable R-channel

	*(volatile unsigned char  *)0x80002b &= 0xCF; //Set reference voltage as 1.428 for R-channel
    *(volatile unsigned char  *)0x80002b &= 0xFC; //Set reference voltage as 1.428 for M-channel

	*(volatile unsigned char  *)0x80003C &= 0xC7; //Set adc resolution to 7 bits for M-channel
    *(volatile unsigned char  *)0x80002f &= 0xC7; //Set adc resolution to 7 bits for R-channel

	*(volatile unsigned char  *)(0x80003c) &= 0xF8;//set sample time for M-channel
}
