...
unsigned char uart_Send(unsigned char* addr)
{
	...
	if (TXDONE) {
		...
		write_reg16(0x800504,(unsigned short)(int)addr);//packet data, start address is sendBuff+1
		STARTTX;
		return 1;
	}
...
void uart_RecBuffInit(unsigned char *recAddr, unsigned short recBuffLen){
	...
	write_reg16(0x800500,(unsigned short)(int)(recAddr));//set receive buffer address
