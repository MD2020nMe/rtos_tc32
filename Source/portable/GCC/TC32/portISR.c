...
/*
	...
	+ The critical section management functions have been changed.  These no
	  ...
	  The functions are now also the same for both TC32 and THUMB modes.
...
__attribute__((naked)) void vPortIRQHandler(void )  
{
	portSAVE_CONTEXT();
...
void vPortYieldProcessor( void )
{
	...
	// MD For this, we need to double check!

	//portDEBUG_PRINTF("Yielding processor ..\n");

	/* Perform the context switch.  First save the context of the current task. */
	portSAVE_CONTEXT();

	/* Find the highest priority task that is ready to run. */
	//__asm volatile ( "tjl vTaskSwitchContext" );
	vTaskSwitchContext();
...
void vPortEnterCritical( void )
{
	...
	ulCriticalNesting++;
	//portDEBUG_PRINTF("Entering critical nested(%d)\n", (int)ulCriticalNesting);
}
