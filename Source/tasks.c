...
unsigned long  debug_flag;
	...
	portDEBUG_PRINTF("pcName %s\n", pcName);
	portDEBUG_PRINTF("pvParameters %04x\n", pvParameters);
	portDEBUG_PRINTF("uxPriority  %d\n", uxPriority);
	if (pvParameters != NULL) {
		portDEBUG_PRINTF("pvParameters %d\n", *(unsigned long *)pvParameters);
	}
		...
		if( pxNewTCB != NULL )
		{
			...
			portDEBUG_PRINTF("xCreateTask [%s] pxNewTCB:%04x  pxStack:%04x StackDepth=%d\n",pcName,pxNewTCB,pxNewTCB->pxStack,usStackDepth);
...
UBaseType_t x;
	...
	portDEBUG_PRINTF("New Task pcName %s\n", pcName);
	portDEBUG_PRINTF("New Task pvParameters %04x\n", pvParameters);
	if (pvParameters != NULL) {
		portDEBUG_PRINTF("pvParameters %d\n", *(unsigned long *)pvParameters);
	}
	...
	#else /* portSTACK_GROWTH */
	{
		...
		portDEBUG_PRINTF("Task[%s], TopofStack %04x EndofStack %04x\n", pcName, pxTopOfStack, pxNewTCB->pxEndOfStack);
	}
	#endif /* portSTACK_GROWTH */
...
/*-----------------------------------------------------------*/
	...
	BaseType_t xYieldRequired = pdFALSE;
		...
		taskEXIT_CRITICAL();
		portDEBUG_PRINTF("vTaskPrioritySet Task[%s] new priority =  %d\n", pxTCB->pcTaskName, uxNewPriority);
	}
...
/*-----------------------------------------------------------*/
	...
	TCB_t *pxTCB;
		...
		taskENTER_CRITICAL();
		{
			...
			//portDEBUG_PRINTF("suspend test:%4x\n",xSuspendedTaskList);
	...
	//	portDEBUG_PRINTF("pxTCB=%04x,pxCurrentTCB=%04x\n",pxTCB,pxCurrentTCB);
		if( pxTCB == pxCurrentTCB )
		{
		    //portDEBUG_PRINTF("suspend test\n");
			if( xSchedulerRunning != pdFALSE )
			{
				...
				//portDEBUG_PRINTF("suspend:pre switch!%04x\n",pxCurrentTCB);
				configASSERT( uxSchedulerSuspended == 0 );
				portYIELD_WITHIN_API();
				//portDEBUG_PRINTF("suspend:after switch!%04x\n",pxCurrentTCB);
			...
			taskENTER_CRITICAL();
			{
				if( prvTaskIsTaskSuspended( pxTCB ) != pdFALSE )
				{
					...
					if( pxTCB->uxPriority >= pxCurrentTCB->uxPriority )
					{
						...
						next yield. */
						//portDEBUG_PRINTF("resume pre switch->%4x\n",pxCurrentTCB);
						taskYIELD_IF_USING_PREEMPTION();
						//portDEBUG_PRINTF("resume after switch->%4x\n",pxCurrentTCB);
					}
					else
					{
						mtCOVERAGE_TEST_MARKER();
...
BaseType_t xReturn;
	...
	#else
	{
		portDEBUG_PRINTF("Creating IDLE task\n");
	...
	if( xReturn == pdPASS )
	{
		...
		if( xPortStartScheduler() != pdFALSE )
		{
			...
			portDEBUG_PRINTF("StartScheduler returned! that's ERROR\n");
		}
		else
		{
...
static portTASK_FUNCTION( prvIdleTask, pvParameters )
{
	...
	for( ;; )
	{
		...
		#if ( ( configUSE_PREEMPTION == 1 ) && ( configIDLE_SHOULD_YIELD == 1 ) )
		{
			...
			{
				//portDEBUG_PRINTF("Idel yielding\n");
				taskYIELD();
			}
			else
			{
				//portDEBUG_PRINTF("Idel not yielding\n");
				mtCOVERAGE_TEST_MARKER();
			}
		}
